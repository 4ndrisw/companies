...
//$where = ['client_id' => $company->userid];
$where = [];
array_push($where, 'AND client_id=' . $this->ci->db->escape_str($client_id));
array_push($where, 'AND client_type='. '"company"');

//$where = hooks()->apply_filters('companies_staff_table_sql_where', []);
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'last_login') {
            ...
            $_data = '<div class="onoffswitch">
                <input type="checkbox" ' . (($aRow['staffid'] == get_staff_user_id() || (is_admin($aRow['staffid']) || !has_permission('pengguna', '', 'edit')) && !is_admin()) ? 'disabled' : '') . ' data-switch-url="' . admin_url() . 'companies/staff/change_staff_state" name="onoffswitch" class="onoffswitch-checkbox" id="c_' . $aRow['staffid'] . '" data-id="' . $aRow['staffid'] . '" ' . $checked . '>
            ...
            if($has_pengguna_edit){
                $_data = '<a href="' . admin_url('companies/staff/profile/' . $aRow['staffid']) . '">' . staff_profile_image($aRow['staffid'], [
                    'staff-profile-image-small',
                    ]) . '</a>';
                $_data .= ' <a href="' . admin_url('companies/staff/member/' . $aRow['staffid']) . '">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>';

                $_data .= '<div class="row-options">';
                $_data .= '<a href="' . admin_url('companies/staff/member/' . $aRow['staffid']) . '">' . _l('view') . '</a>';
    ...
    $row = hooks()->apply_filters('companies_staff_table_row', $row, $aRow);
