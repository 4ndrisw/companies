 ...
 * @return mixed
 */
function get_customer_data($type, $rel_id = '', $extra = [])
{
    $CI = & get_instance();
    $q  = '';
    if ($CI->input->post('q')) {
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($rel_id != '') {
            ...
            $data = $CI->programs_model->get($rel_id);
        } else {
            $where_programs = '';
            if ($CI->input->post('customer_id')) {
                $where_programs .= 'clientid=' . $CI->db->escape_str($CI->input->post('customer_id'));
            }
            $search = $CI->misc_model->_search_programs($q, 0, $where_programs);
            $data   = $search['result'];
        }
    } elseif ($type == 'staff') {
        if ($rel_id != '') {
    ...
    $data = hooks()->apply_filters('get_customer_data', $data, compact('type', 'rel_id', 'extra'));
 ...
 * @return string
 */
function init_customer_options($data, $type, $rel_id = '')
{
    $_data = [];

    $has_permission_programs_view  = has_permission('programs', '', 'view');
    ...
    foreach ($data as $relation) {
        $relation_values = get_customer_values($relation, $type);
        if ($type == 'program') {
            if (!$has_permission_programs_view) {
                if (!$CI->programs_model->is_member($relation_values['id']) && $rel_id != $relation_values['id']) {
                    continue;
                }
            }
        } elseif ($type == 'lead') {
    ...
    $_data = hooks()->apply_filters('init_customer_options', $_data, compact('data', 'type', 'rel_id'));

    return $_data;
}


function companies_notification()
{
    $CI = &get_instance();
    $CI->load->model('companies/companies_model');
    $companies = $CI->companies_model->get('', true);
    /*
    foreach ($companies as $goal) {
        $achievement = $CI->companies_model->calculate_goal_achievement($goal['id']);

        if ($achievement['percent'] >= 100) {
            if (date('Y-m-d') >= $goal['end_date']) {
                if ($goal['notify_when_achieve'] == 1) {
                    $CI->companies_model->notify_staff_members($goal['id'], 'success', $achievement);
                } else {
                    $CI->companies_model->mark_as_notified($goal['id']);
                }
            }
        } else {
            ...
            if (date('Y-m-d') > $goal['end_date']) {
                if ($goal['notify_when_fail'] == 1) {
                    $CI->companies_model->notify_staff_members($goal['id'], 'failed', $achievement);
                } else {
                    $CI->companies_model->mark_as_notified($goal['id']);
                }
            }
...
/**
 * Function that return company item taxes based on passed item id
 * @param  mixed $itemid
 * @return array
 */
function get_company_item_taxes($itemid)
{
    $CI = &get_instance();
    $CI->db->where('itemid', $itemid);
    $CI->db->where('rel_type', 'company');
...
/**
 * Get Company short_url
 * @since  Version 2.7.3
 * @param  object $company
 * @return string Url
 */
function get_company_shortlink($company)
{
    $long_url = site_url("company/{$company->id}/{$company->hash}");
    if (!get_option('bitly_access_token')) {
        return $long_url;
    }

    // Check if company has short link, if yes return short link
    if (!empty($company->short_link)) {
        return $company->short_link;
    }

    // Create short link and return the newly created short link
    $short_link = app_generate_short_link([
        'long_url'  => $long_url,
        'title'     => format_company_number($company->id)
    ]);

    if ($short_link) {
        $CI = &get_instance();
        $CI->db->where('id', $company->id);
...
/**
 * Check company restrictions - hash, clientid
 * @param  mixed $id   company id
 * @param  string $hash company hash
 */
function check_company_restrictions($id, $hash)
{
    $CI = &get_instance();
    ...
    if (!is_client_logged_in() && !is_staff_logged_in()) {
        if (get_option('view_company_only_logged_in') == 1) {
            redirect_after_login_to_current_url();
            redirect(site_url('authentication/login'));
        }
    }
    $company = $CI->companies_model->get($id);
    if (!$company || ($company->hash != $hash)) {
        show_404();
    }
    // Do one more check
...
/**
 * Check if company email template for expiry reminders is enabled
 * @return boolean
 */
function is_companies_email_expiry_reminder_enabled()
{
    return total_rows(db_prefix() . 'emailtemplates', ['slug' => 'company-expiry-reminder', 'active' => 1]) > 0;
}

/**
 * Check if there are sources for sending company expiry reminders
 * Will be either email or SMS
 * @return boolean
 */
function is_companies_expiry_reminders_enabled()
{
    return is_companies_email_expiry_reminder_enabled() || is_sms_trigger_active(SMS_TRIGGER_SCHEDULE_EXP_REMINDER);
}

/**
 * Return RGBa company state color for PDF documents
 * @param  mixed $state_id current company state
 * @return string
 */
function company_state_color_pdf($state_id)
{
    if ($state_id == 1) {
    ...
    return hooks()->apply_filters('company_state_pdf_color', $stateColor, $state_id);
}

/**
 * Format company state
 * @param  integer  $state
 * @param  string  $classes additional classes
 * @param  boolean $label   To include in html label or not
 * @return mixed
 */
function format_company_state($state, $classes = '', $label = true)
{
    $id          = $state;
    $label_class = company_state_color_class($state);
    $state      = company_state_by_id($state);
    if ($label == true) {
        return '<span class="label label-' . $label_class . ' ' . $classes . ' s-state company-state-' . $id . ' company-state-' . $label_class . '">' . $state . '</span>';
    }
...
/**
 * Return company state translated by passed state id
 * @param  mixed $id company state id
 * @return string
 */
function company_state_by_id($id)
{
    $state = '';
    if ($id == 1) {
        $state = _l('company_state_draft');
    } elseif ($id == 2) {
        $state = _l('company_state_sent');
    } elseif ($id == 3) {
        $state = _l('company_state_declined');
    } elseif ($id == 4) {
        $state = _l('company_state_accepted');
    } elseif ($id == 5) {
        // state 5
        $state = _l('company_state_expired');
    } else {
        if (!is_numeric($id)) {
            if ($id == 'not_sent') {
    ...
    return hooks()->apply_filters('company_state_label', $state, $id);
}

/**
 * Return company state color class based on twitter bootstrap
 * @param  mixed  $id
 * @param  boolean $replace_default_by_muted
 * @return string
 */
function company_state_color_class($id, $replace_default_by_muted = false)
{
    $class = '';
    if ($id == 1) {
        $class = 'default';
    ...
    return hooks()->apply_filters('company_state_color_class', $class, $id);
}

/**
 * Check if the company id is last invoice
 * @param  mixed  $id companyid
 * @return boolean
 */
function is_last_company($id)
{
    $CI = &get_instance();
    $CI->db->select('userid')->from(db_prefix() . 'clients')->order_by('userid', 'desc')->limit(1);
    $query            = $CI->db->get();
    $last_company_id = $query->row()->userid;
    if ($last_company_id == $id) {
        return true;
    }
...
/**
 * Format company number based on description
 * @param  mixed $id
 * @return string
 */
function format_company_number($id)
{
    $CI = &get_instance();
    ...
    $number = company_number_format($company->number, $company->number_format, $company->prefix, $company->datecreated);

    return hooks()->apply_filters('format_company_number', $number, [
        'userid'       => $id,
        'company' => $company,
    ]);
}
...
function company_number_format($number, $format, $applied_prefix, $date)
{
    ...
    return hooks()->apply_filters('company_number_format', $number, [
        'format'         => $format,
        'date'           => $date,
...
/**
 * Calculate companies percent by state
 * @param  mixed $state          company state
 * @return array
 */
function get_companies_percent_by_state($state, $program_id = null)
{
    ...
    if (!$has_permission_view) {
        $where .= get_companies_where_sql_for_staff(get_staff_user_id());
    }
    ...
    $total_companies = total_rows(db_prefix() . 'clients', $where);
    ...
    $percent                 = ($total_companies > 0 ? number_format(($total_by_state * 100) / $total_companies, 2) : 0);
...
function get_companies_where_sql_for_staff($staff_id)
{
    $CI = &get_instance();
    $has_permission_view_own             = has_permission('companies', '', 'view_own');
    $allow_staff_view_companies_assigned = get_option('allow_staff_view_companies_assigned');
    $whereUser                           = '';
    if ($has_permission_view_own) {
        $whereUser = '((' . db_prefix() . 'companies.addedfrom=' . $CI->db->escape_str($staff_id) . ' AND ' . db_prefix() . 'companies.addedfrom IN (SELECT staff_id FROM ' . db_prefix() . 'staff_permissions WHERE feature = "companies" AND capability="view_own"))';
        if ($allow_staff_view_companies_assigned == 1) {
...
/**
 * Check if staff member have assigned companies / added as sale agent
 * @param  mixed $staff_id staff id to check
 * @return boolean
 */
function staff_has_assigned_companies($staff_id = '')
{
    $CI       = &get_instance();
    $staff_id = is_numeric($staff_id) ? $staff_id : get_staff_user_id();
    $cache    = $CI->app_object_cache->get('staff-total-assigned-companies-' . $staff_id);

    if (is_numeric($cache)) {
        $result = $cache;
    } else {
        $result = total_rows(db_prefix() . 'clients', ['assigned' => $staff_id]);
        $CI->app_object_cache->add('staff-total-assigned-companies-' . $staff_id, $result);
    }

    return $result > 0 ? true : false;
}
/**
 * Check if staff member can view company
 * @param  mixed $id company id
 * @param  mixed $staff_id
 * @return boolean
 */
function user_can_view_company($id, $staff_id = false)
{
    $CI = &get_instance();

    $staff_id = $staff_id ? $staff_id : get_staff_user_id();

    if (has_permission('companies', $staff_id, 'view')) {
        return true;
    }
    ...
    if(is_client_logged_in()){
        ...
        // Do one more check
        if (get_option('view_companyt_only_logged_in') == 1) {
            if ($company->clientid != get_client_user_id()) {
                show_404();
            }
        }
    ...
    if ((has_permission('companies', $staff_id, 'view_own') && $company->addedfrom == $staff_id)
        || ($company->assigned == $staff_id && get_option('allow_staff_view_companies_assigned') == '1')
    ) {
        return true;
    }
...
/**
 * Prepare general company pdf
 * @since  Version 1.0.2
 * @param  object $company company as object with all necessary fields
 * @param  string $tag tag for bulk pdf exporter
 * @return mixed object
 */
function company_pdf($company, $tag = '')
{
    return app_pdf('company',  module_libs_path(COMPANIES_MODULE_NAME) . 'pdf/Company_pdf', $company, $tag);
}
 ...
 * @return mixed object
 */
function company_office_pdf($company, $tag = '')
{
    return app_pdf('company',  module_libs_path(COMPANIES_MODULE_NAME) . 'pdf/Company_office_pdf', $company, $tag);
}
 ...
 * @param  object  $transaction   e.q. invoice, company from database result row
 * @param  string  $type          type, e.q. invoice, company, proposal
 * @param  string  $for           where the items will be shown, html or pdf
 * @param  boolean $admin_preview is the preview for admin area
 * @return object
 */
function get_company_items_table_data($transaction, $type, $for = 'html', $admin_preview = false)
{
    include_once(module_libs_path(COMPANIES_MODULE_NAME) . 'Company_items_table.php');

    $class = new Company_items_table($transaction, $type, $for, $admin_preview);
    ...
    if (!$class instanceof App_items_table_template) {
        show_error(get_class($class) . ' must be instance of "Company_items_template"');
    }
...
/**
 * Add new item do database, used for proposals,companies,credit notes,invoices
 ...
 */
function add_new_company_item_post($item, $rel_id, $rel_type)
{
...
/**
 * Update company item from $_POST 
 ...
 * @return boolean
 */
function update_company_item_post($item_id, $data, $field = '')
{
    $update = [];
    if ($field !== '') {
 ...
 * @return array
 */
function company_mail_preview_data($template, $customer_id_or_email, $mailClassParams = [])
{
    $CI = &get_instance();
 ...
 * @return string
 */
function get_company_upload_path($type=NULL)
{
   $type = 'company';
...
/**
 * Remove and format some common used data for the company feature eq invoice,companies etc..
 * @param  array $data $_POST data
 * @return array
 */
function _format_data_company_feature($data)
{
    foreach (_get_company_feature_unused_names() as $u) {
...
if (!function_exists('format_company_info')) {
    /**
     * Format company info format
     * @param  object $company company from database
     * @param  string $for      where this info will be used? Admin area, HTML preview?
     * @return string
     */
    function format_company_info($company, $for = '')
    {
        $format = get_option('company_info_format');
        $countryCode = '';
        $countryName = '';

        if ($country = get_country($company->country)) {
            $countryCode = $country->iso2;
            $countryName = $country->short_name;
        }

        $companyTo = '<b>' . $company->company . '</b>';

        if ($for == 'admin') {
            $hrefAttrs = '';
            $hrefAttrs = ' href="' . admin_url('clients/client/' . $company->userid) . '" data-toggle="tooltip" data-title="' . _l('client') . '"';
            $companyTo = '<a' . $hrefAttrs . '>' . $companyTo . '</a>';
        }
        ...
        $format = _apps_format_replace('company_name', $companyTo, $format);
        $format = _apps_format_replace('address', $company->address . ' ' . $company->city, $format);

        $format = _apps_format_replace('city', NULL, $format);
        $format = _apps_format_replace('state', $company->state . ' ' . $company->zip, $format);
        ...
        $customFieldsProposals = get_custom_fields('company', $whereCF);
        ...
        return hooks()->apply_filters('company_info_text', $format, ['company' => $company, 'for' => $for]);
    }
}
 ...
 * @return array
 */
function _get_company_feature_unused_names()
{
    return [
        'taxname', 'description',
 ...
 * @param  string $rel_type item relation eq. invoice, company
 * @return boolena
 */
function handle_removed_company_item_post($id, $rel_type)
{
    $CI = &get_instance();
 ...
 * @return boolean
 */
function program_has_companies($program_id)
{
    $totalProgramsCompanyd = total_rows(db_prefix() . 'clients', 'program_id=' . get_instance()->db->escape_str($program_id));

    return ($totalProgramsCompanyd > 0 ? true : false);
}


function is_staff_related_to_company($client_id){
    $CI = &get_instance();
 ...
 * @return mixed
 */
function get_primary_staff_client_id($client_id)
{
    $CI = &get_instance();
    $CI->db->where('client_id', $client_id);
    $CI->db->where('is_primary', 1);
    //$sql = $CI->db->get_compiled_select('staff');
